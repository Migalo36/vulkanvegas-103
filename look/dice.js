...
function denyWager(message){verdictView.textContent=message;wagerInput.classList.remove('rl-deny-24e6385d');void wagerInput.offsetWidth;wagerInput.classList.add('rl-deny-24e6385d');}
...
rolling=true;rollBtn.disabled=true;clearFlash();wagerInput.classList.remove('rl-deny-24e6385d');purse=round2(purse-wager);savePurse();renderPurse();var target=currentTarget();var prizeRate=payoutFor(target);verdictView.textContent='The die is rolling…';dieView.classList.add('rl-spin-24e6385d');scrambleTimer=window.setInterval(function(){dieView.textContent=String(Math.floor(Math.random()*100)+1);},SCRAMBLE_TICK_MS);window.setTimeout(function(){settleRoll(wager,target,prizeRate);},SCRAMBLE_MS);}
...
var brandEl=document.querySelector('.rl-brand-24e6385d');var echoEl=grab('rl-echo');if(brandEl&&echoEl){echoEl.textContent=brandEl.textContent.replace(/\s+/g,' ').trim();}
